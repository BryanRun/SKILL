...
| **贴到 Gerrit** | 以评审评论形式贴出 | 以 Checklist 评论形式贴出 |
| **是否可修改** | 根据代码评审结果生成 | **内容完全固化，不可修改** |

**关键原则**：
1. **Step 2 的评审结论不能替代 Step 3 的 Checklist**
2. **Step 3 的 Checklist 不能省略或简化为评审结论**
3. **Checklist 的 12 项检查内容完全固化**，不可根据评审结果增删或修改检查项的文字描述
4. Claude 只能标注每项的状态（✓ / x / o），不能修改检查项本身的内容
